...
import json
import torch
from openpifpaf.encoder.midas.transforms import Resize,NormalizeImage,PrepareForNet
import torch.utils.data
from PIL import Image

from openpifpaf import transforms, utils
from openpifpaf.plugins.centernet.utils import read_pfm
from torchvision.transforms import Compose
# from openpifpaf.openpifpaf.transforms import Compose
...
class VG(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, depth_dir, *,
                 preprocess=None, split="train", num_im=-1, num_val_im=-1,
                 filter_duplicate_rels=True, filter_non_overlap=True, filter_empty_rels=True, use_512=False,
                 eval_mode=False, convert_rgb=False):
        ...
        self.width = self.im_h5['image_widths']
        self.height = self.im_h5['image_heights']
        self.original_width = self.im_h5['original_widths']
        self.original_height = self.im_h5['original_heights']
    ...
    def __getitem__(self, index):
        ...
        image = Image.fromarray(self._im_getter(index))
        # depth_image=copy.deepcopy(image)
        # depth_image=self.transforms({'image':np.array(depth_image).transpose((1,2,0))})
        ...
        # dpth_file_path = os.path.join(self.depth_dir, f"{image_id}-dpt_beit_large_512.pfm")
        dpth_file_path = os.path.join(self.depth_dir, f"{image_id}.png")
        if os.path.exists(dpth_file_path):
        #     depth_information = np.zeros((512, 512), dtype=np.float32)
        #     original_depth, scale = read_pfm(os.path.join(self.depth_dir, f"{image_id}-dpt_beit_large_512.pfm"))
        #     if not np.any(np.isnan(original_depth)):
        #         original_depth = (original_depth - np.min(original_depth)) / (
        #                     np.max(original_depth) - np.min(original_depth))*255
        ...
        #     else:
        #         print("nan parameter appear")
            depth_information = Image.fromarray(cv2.imread(dpth_file_path).astype(np.uint8))
        else:
            print(f"file {dpth_file_path} doesn't exist")
            depth_information = Image.fromarray(np.zeros((512, 512,3), dtype=np.uint8))
        # depth_information=depth_information.transpose(2,0,1)
        # height, weight = depth_information.shape[:2]
        # depth_information = cv2.resize(depth_information, (512 // 16, 512 // 16))
        # depth_image=torch.from_numpy(depth_information)
        meta = {
            'dataset_index': index,
            'image_id': image_id,
            'file_name': image_id,
        ...
        if not self.preprocess is None:
            if self.eval_mode:
                ...
                image, anns, meta,depth_information = self.preprocess(image, anns, meta,depth_information)
            else:
                # preprocess image and annotations
                image, anns, meta,depth_information = self.preprocess(image, anns, meta,depth_information)
        # anns.append(depth_information)
        ...
        LOG.debug(meta)
        # if np.any(np.isnan(depth_information)):
        #     print("depth information has nan")
        if self.eval_mode:
            return image, (anns, anns_gt), meta, depth_information
        return image, anns, meta,depth_information
