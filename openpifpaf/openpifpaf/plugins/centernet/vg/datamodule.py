...
class VGModule(openpifpaf.datasets.DataModule):
        ...
        if self.special_preprocess:
            return openpifpaf.transforms.Compose([
                ...
                openpifpaf.transforms.RandomApply(
                    Raf_HFlip(BBOX_KEYPOINTS, BBOX_HFLIP, REL_CATEGORIES, REL_CATEGORIES_FLIP), 0.5),
                # rescale_t,
                openpifpaf.transforms.RescaleRelative(scale_range=(0.8 * self.rescale_images,
                                                                   1.3 * self.rescale_images),
                                                      absolute_reference=self.square_edge),
    ...
    def _eval_preprocess(self):
        ...
        if self.prior_token:
            ...
            if not 'raf' in self.prior_token:
                ...
                mask_encoder = openpifpaf.transforms.Encoders(
                    [CenterNetEncoder(self.head_metas[0]), Raf_CNEncoder(self.head_metas[1])], join=True)
        return openpifpaf.transforms.Compose([
            openpifpaf.transforms.NormalizeAnnotations(),
            rescale_t,
            padding_t,
    ...
    def _get_fg_matrix(self):
        ...
        self.head_metas[-1].fg_matrix, self.head_metas[-1].bg_matrix, self.head_metas[
            -1].smoothing_pred = train_data.get_frequency_prior(self.obj_categories, self.rel_categories)
    ...
    def eval_loader(self):
        eval_data = VG(
            ...
            use_512=self.vg_512,
            eval_mode=True,
            convert_rgb=self.vg_rgb,depth_dir='data/hha'
        )
        if not self.only_det:
            self._get_fg_matrix()
    ...
    def metrics(self):
        ...
        if self.only_det:
            return [metric.VG(obj_categories=self.obj_categories, rel_categories=self.rel_categories, mode='sgdet',
                              iou_types=['bbox'], vg_eval=eval_data)]
        return [metric.VG(obj_categories=self.obj_categories, rel_categories=self.rel_categories, mode='sgdet',
                          iou_types=['bbox', 'relations'] if (not self.only_det) else ['bbox'], vg_eval=eval_data),
                metric.VG(obj_categories=self.obj_categories, rel_categories=self.rel_categories, mode='sgcls',
                          iou_types=['bbox', 'relations'] if (not self.only_det) else ['bbox'], vg_eval=eval_data),
                metric.VG(obj_categories=self.obj_categories, rel_categories=self.rel_categories, mode='predcls',
                          iou_types=['bbox', 'relations'] if (not self.only_det) else ['bbox'], vg_eval=eval_data)
                ]
