...
class Attention(nn.Module):
    def __init__(
            self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., with_cls=False,
            require_query=False
    ):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.input_query = require_query
    ...
    def forward(self, x, mask=None, query=None):
        ...
        if self.input_query and query is not None:
            kv = self.kv(x).reshape(B, N, 2, self.num_heads, C // self.num_heads)
            q = self.q(query).reshape(B, N, 1, self.num_heads, C // self.num_heads)
            qkv = torch.cat([q, kv], dim=2).permute(2, 0, 3, 1, 4)
        elif not self.input_query and not query:
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        else:
            raise NotImplementedError("input requires query but  query is None")
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, with_cls=False, require_query=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            with_cls=with_cls, require_query=require_query)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

    def forward(self, x, mask=None, return_attention=False, query=None):
        y, attn = self.attn(self.norm1(x), mask=mask, query=query)
        if return_attention:
            return attn
...
class TransformerRefine(nn.Module):
    ...
    def __init__(self, in_channels, embed_dim=512, out_channels=0, depth=6, num_heads=8, mlp_ratio=4.,
                 ...
                 norm_layer=nn.LayerNorm, pos_encoding_type="fourierv2", require_query=False, **kwargs):
        super().__init__()
        ...
        if require_query:
            self.query_proj = nn.Conv2d(in_channels, embed_dim, kernel_size=1)
    ...
    def forward(self, x, prior_info=None, query=None):
        x = self.input_proj(x)
        B, C, Hp, Wp = x.shape
        x = x.flatten(2).permute(0, 2, 1)
        if self.require_query:
            query = self.query_proj(query).flatten(2).permute(0,2,1)
        else:
            query = None
        ...
        # pos_encoding.gather(1,prior_info[1].unsqueeze(-1).expand(-1, -1, C))
        if not prior_info is None:
            prior_tokens = prior_info[0]
            prior_dim = prior_tokens.shape[1]
            prior_pos = pos_encoding.gather(1, prior_info[1].unsqueeze(-1).expand(-1, -1, C))
            prior_tokens = prior_tokens + prior_pos
            x = torch.cat([x, prior_tokens], dim=1)
