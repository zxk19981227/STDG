...
class CifDetRaf_CN(Decoder):
    ...
    max_score = True
    timing = True
    fpn_interval_bbox = {'0': [0, 32 ** 2], '1': [32 ** 2, 64 ** 2], '2': [64 ** 2, 128 ** 2],
                         '3': [128 ** 2, 512 ** 2]}
    ...
    def __init__(self,
        ...
        if self.cifdet_visualizers is None:
            chosen_visualizer = visualizer.CifDet if isinstance(cifdet_metas[-1],
                                                                headmeta.CifDet_CN) else visualizer_centernet.CenterNet
        ...
        if self.raf_visualizers is None:
            chosen_vis_raf = visualizer_centernet.Raf if isinstance(self.raf_metas[-1],
                                                                    headmeta.Raf_CNs) else visualizer_raf.Raf
    ...
    def factory(cls, head_metas):
        ...
        for head_meta in head_metas:
            if isinstance(head_meta, headmeta.CifDet_CN) or isinstance(head_meta, headmeta.CenterNet) or isinstance(
                    head_meta, headmeta.CenterNet_FPN):
                cifdet_metas.append(head_meta)
            elif isinstance(head_meta, headmeta.Raf_CN) or isinstance(head_meta, headmeta.Raf_CNs) or isinstance(
                    head_meta, headmeta.Raf_CAF) or isinstance(head_meta, headmeta.Raf_GDeform) or isinstance(head_meta,
                                                                                                              headmeta.Raf_FPN):  # or isinstance(meta_next, headmeta.Raf_dcn)
    ...
    def solve_RAFCN_fasterer_cuda(self, raf_analyzer, annotations_det, predcls=False):
        ...
        for ann in annotations_det:
            ...
            pred_bboxes.append(torch.tensor(
                [[bbox[0] + bbox[2] / 2.0, bbox[1] + bbox[3] / 2.0, bbox[2], bbox[3], ann.category_id, ann.score]],
        ...
        if False:
            ...
            weights = self.pred_dist[
                pred_bboxes[id_s_perRel[mask_idx], 4].astype(int) - 1, pred_bboxes[id_o_perRel[mask_idx], 4].astype(
                    int) - 1, sorted_raf[mask_idx, 4].astype(int) + 1]
    ...
    def __call__(self, fields, initial_annotations=None, meta=None):
        ...
        if self.pred_dist is None and not self.raf_metas[-1].fg_matrix is None:
            ...
            self.prob_dist_rel = self.raf_metas[-1].fg_matrix / (
                        self.raf_metas[-1].fg_matrix.sum(2)[:, :, None] + 1e-03)
        ...
        if gt_anns:
            return (annotations, annotations_det), (annotations_predcls, anns_det_gt), (
            annotations_sgcls, anns_det_sgcls)
    ...
    def overlap_gt_pred(self, anns_det_gt, hm):
        ...
        for ann in anns_det_gt:
            ...
            hm_max = np.amax(hm[:, max(0, (ct[1] - 1)):min(hm.shape[1], (ct[1] + 2)),
                             max(0, (ct[0] - 1)):min(hm.shape[2], (ct[0] + 2))], axis=(1, 2))
