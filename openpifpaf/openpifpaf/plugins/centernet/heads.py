...
class deform_convolution(nn.Module):
    ...
    def forward(self, x):
        ...
        if pre_offset is not None:
            relu=torch.cat([relu,offset,pre_offset],dim=1)
        else:
            relu=torch.cat([relu,offset],dim=1)
        return relu
...
class CenterNetHead(HeadNetwork):
    ...
    def __init__(self,
                 meta: headmeta.Base,
                 in_features, *,
                 kernel_size=1, padding=0, dilation=1,require_query=False):
        ...
        self.reqire_query=require_query
        self.sigmod_=torch.nn.Sigmoid()
        # self.extract_information=torch.nn.Conv2d(256,256*3,padding=0,dilation=1,kernel_size=1)
        if self.downup4:
            self.downsample = torch.nn.Conv2d(in_features, in_features, (4, 4), stride=4)
            self.upsample = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False)
        if self.deform4_head:
            if self.joint_transformer:
                in_features=256
        ...
        if self.joint_transformer or self.solo_transformer:
            ...
            if self.meta.kernel_1:
                ...
                self.wh = make_5deep_layer(
                                in_features, 256, 2 * (meta.upsample_stride ** 2), with_bn=self.withBN)
                self.depth = make_5deep_layer(
                                in_features, 256, 1 * (meta.upsample_stride ** 2), with_bn=self.withBN
                )
            else:
                ...
                self.wh = make_kp_layer(
                                in_features, 256, 2 * (meta.upsample_stride ** 2), with_bn=self.withBN)
                self.depth = make_kp_layer(
                                in_features, 256, 1 * (meta.upsample_stride ** 2), with_bn=self.withBN)
        elif self.single_conv:
            self.obj_head = torch.nn.Conv2d(in_features, meta.n_fields + 4, (1, 1))
        else:
            self.transformer = TransformerRefine(in_channels=in_features, out_channels=256, embed_dim=256, mlp_ratio=8., num_heads=8, pos_encoding_type=self.positional_encoding, drop_path_rate=0.0,require_query=require_query)
            # if require_query:
            #     self.depth_transformer=TransformerRefine(in_channels=in_features, out_channels=256, embed_dim=256, mlp_ratio=8., num_heads=8, pos_encoding_type=self.positional_encoding, drop_path_rate=0.0)
    ...
    def forward(self, x, targets=None,extra=None):  # pylint: disable=arguments-differ
        tensor_toconcat = []
        prior_info = None
        ...
        if self.downup4:
            x = self.downsample(x)
        if self.deform4_head:
            tmp_x = self.pre_head(x)
            deform4offset = tmp_x[:,256:, :, :]
            x=tmp_x[:,:256,:,:]
        else:
            deform4offset = None
        if self.single_conv or self.joint_transformer or self.solo_transformer:
            ...
            # x = torch.cat([x, depth_information], dim=1)
            x = self.transformer(x, prior_info=prior_info)#,query=depth_information)
            # print(f"x shape is {x.shape}")
            # encoded_depth_info=self.combine_depth_and_information0(depth_information)
            # encoded_depth_info=self.combine_depth_and_information1(encoded_depth_info)

            # cross_feature=torch.cat([encoded_depth_info,x,x.mul(encoded_depth_info)],dim=1)
            # use the channel wise attention to extracted useful information
            # encoded_attention=self.compress_information(cross_feature)
            # encoded_attention=self.extract_information(encoded_attention)
            # encoded_attention=self.sigmod_(encoded_attention)
            # weighted_x=torch.mul(encoded_attention,cross_feature)
            # x=torch.cat([x,depth_information],dim=1)
            # print("using depth")
            # exit(0)
            # x=self.combine_depth_and_information(weighted_x)
            # x=self.combine_depth_and_information2(weighted_x)
            if self.downup4:
                x = self.upsample(x)
            x = self.obj_head(x)
            x[:,:self.meta.n_fields, :,:] = torch.sigmoid(x[:,:self.meta.n_fields, :,:])
...
class CompositeField3_singleDeform_fourier(HeadNetwork):
        ...
        if self.multipleheads:
            ...
            if self.meta.refine_feature or self.meta.biggerparam or self.meta.deform_conv:
                ...
                if self.use_concat:
                    ...
                    input_dim=256
                self.raf_head = torch.nn.Conv2d(input_dim, out_features,
            ...
            # self.raf_head = make_hm_layer(
            #                 in_features_refined, 256, out_features, with_bn=self.withBN)
        # self.combine_depth_and_information0=torch.nn.Conv2d(2048,1024,kernel_size=1,padding=0,dilation=1)
        # self.combine_depth_and_information1=torch.nn.Conv2d(1024,256,kernel_size=1,padding=0,dilation=1)
        # self.combine_depth_and_information2=torch.nn.Conv2d(256*3,256,kernel_size=1,padding=0,dilation=1)
        self.global_pool=torch.nn.AvgPool2d((7,7))
        # self.compress_information=torch.nn.Conv2d(256,256,padding=0,dilation=1,kernel_size=1)
        self.sigmod_=torch.nn.Sigmoid()
        # self.extract_information=torch.nn.Conv2d(256,256*3,padding=0,dilation=1,kernel_size=1)
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--cf3-deform-use-concat',
                            default=False, action='store_true',
                            help='Use concat instead of sum')
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        cls.use_concat=args.cf3_deform_use_concat
    ...
    def forward(self, x, extra=None,targets=None):  # pylint: disable=arguments-differ
