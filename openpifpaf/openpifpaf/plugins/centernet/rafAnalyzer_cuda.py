...
class RafAnalyzer:
    ...
    def fill_single(self, all_fields, meta: headmeta.Raf_CN):
        ...
        # print(f'raf shape is {raf.shape}')
        print(f"shape for torch arnage is {torch.arange(raf.shape[0], device=torch.device('cuda:0'))[(...,) + (None,) * 3].shape}")
        print(f"expanded shape is {[raf.shape[0], 1,*raf.shape[2:]]}")
        raf_idx = torch.arange(raf.shape[0], device=torch.device('cuda:0'))[(...,) + (None,) * 3].expand(raf.shape[0],
                                                                                                         1,
                                                                                                         *raf.shape[2:])
