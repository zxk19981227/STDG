...
class Crop(Preprocess):
    def __init__(self, long_edge, use_area_of_interest=True):
        if isinstance(long_edge, int):
            long_edge = (long_edge, long_edge)
    ...
    def __call__(self, image, anns, meta,depth):
        ...
        image, anns, ltrb,depth = self.crop(image, anns, meta['valid_area'],depth)
        meta['offset'] += ltrb[:2]
    ...
    def area_of_interest(anns, valid_area, edge_length):
        ...
        range_x = max(0, valid_area[2] - edge_length[0])
        range_y = max(0, valid_area[3] - edge_length[1])
    ...
    def crop(self, image, anns, valid_area,depth):
        ...
        w, h = image.size
        padding_w = int(self.long_edge[0] / 2.0)
        padding_h = int(self.long_edge[1] / 2.0)
        x_offset, y_offset = 0, 0
        if w > self.long_edge[0]:
            min_x = int(area_of_interest[0])
            max_x = int(area_of_interest[0] + area_of_interest[2]) - self.long_edge[0]
            if max_x > min_x:
                x_offset = torch.randint(-padding_w + min_x, max_x + padding_w, (1,))
                x_offset = torch.clamp(x_offset, min=min_x, max=max_x).item()
            else:
                x_offset = min_x
        if h > self.long_edge[1]:
            min_y = int(area_of_interest[1])
            max_y = int(area_of_interest[1] + area_of_interest[3]) - self.long_edge[1]
            if max_y > min_y:
                y_offset = torch.randint(-padding_h + min_y, max_y + padding_h, (1,))
        ...
        # crop image
        new_w = min(self.long_edge[0], w - x_offset)
        new_h = min(self.long_edge[1], h - y_offset)
        ...
        return image, anns, np.array(ltrb),depth
