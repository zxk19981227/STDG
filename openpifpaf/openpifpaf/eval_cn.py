...
def count_ops(model, height=640, width=640):
    ...
    depth_input = torch.randn(1,3, height, width, device=device)
    gmacs, params = thop.profile(model, inputs=(dummy_input, depth_input))
...
def evaluate(args):
    ...
    for batch_i, (image_tensors, anns_batch, meta_batch, depth) in enumerate(loader):
        # print(batch)
        # (image_tensors, anns_batch, meta_batch, depth)=batch
        ...
        pred_batch = processor.batch(model, image_tensors, depth=depth,
                                     device=args.device, gt_anns_batch=anns_batch)
        if args.flip_test:
            pred_batch_flipped = processor.batch(model, \
                                                 [torch.flip(image_tensor, (3,)) for image_tensor in
                                                  image_tensors] if isinstance(image_tensors, list) \
        ...
        for pred_idx, (pred, gt_anns, image_meta) in enumerate(zip(pred_batch, anns_batch, meta_batch)):
            if pred and (isinstance(pred[0], list) or isinstance(pred[0], tuple)):
                if isinstance(pred[0], list):
                    ...
                    for i in range(len(pred)):
                        pred[i] = ([ann.inverse_transform(image_meta) for ann in pred[i][0]],
                                   [ann.inverse_transform(image_meta) for ann in pred[i][1]])
                for i in range(2):
            ...
            if args.show_final_image:
                ...
                if len(pred_rel) > 0:
                    fig_file = os.path.join('all-images', interim_folder,
                                            str(image_meta['file_name']) + '_corr_rel.jpg')
                    with show.image_canvas(cpu_image, fig_file=fig_file) as ax:
                        rel_painter.annotations(ax, pred_rel[res], metas=image_meta, gt="_corr",
                ...
                if args.show_final_ground_truth:
                    ...
                    with show.image_canvas(cpu_image, fig_file=fig_file) as ax:
                        rel_painter.annotations(ax, gt_anns[0][0], metas=image_meta, gt="_gt",
                                                interim_folder=interim_folder + "/")
