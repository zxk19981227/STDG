...
class DepthField(HeadNetwork):
    dropout_p=0.0
    def __init__(self,meta: headmeta.Base,
                 in_features, *,
        ...
        # for depth, the output features only have one dimension
        out_features=1
        ...
        if self.meta.n_vectors > 0 and hasattr(self.meta, 'vector_softplus') and any(self.meta.vector_softplus):
            print("Sofplus used for WH")

    @classmethod
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('CompositeField3')
        group.add_argument('--depth-dropout', default=cls.dropout_p, type=float,
                           help='[experimental] zeroing probability of feature in head input')
        assert cls.inplace_ops
        group.add_argument('--depth-no-inplace-ops', dest='cf3_inplace_ops',
                           default=True, action='store_false',
                           help='alternative graph without inplace ops')

    @classmethod
    def configure(cls, args: argparse.Namespace):
        cls.dropout_p = args.depth_dropout
        cls.inplace_ops = args.depth_inplace_ops
    ...
    def forward(self, x):  # pylint: disable=arguments-differ
        ...
        if not self.training and self.inplace_ops:
            ...
            torch.sigmoid_(classes_x)+1e-10
            ...
            # classification
            classes_x = x[:, 0:self.meta.n_confidences]
            classes_x = torch.sigmoid(classes_x)+1e-10
...
class CompositeField3(HeadNetwork):
    ...
    def forward(self, x):  # pylint: disable=arguments-differ
        ...
        # if self.meta.n_vectors > 0 and hasattr(self.meta, 'vector_softplus') and any(self.meta.vector_softplus):
        #     tensor_toconcat = []
        #     tensor_toconcat.append(x[:, :, 0:self.meta.n_confidences])
        #     first_reg_feature = self.meta.n_confidences
        #     for i, do_softplus in enumerate(self.meta.vector_softplus):
        #         reg_x = x[:, :, first_reg_feature + i * 2:first_reg_feature + (i + 1) * 2]
        #         tensor_toconcat.append(reg_x)
        #         if not do_softplus:
        #             continue
        #         tensor_toconcat[-1] = torch.nn.functional.softplus(reg_x)
        #     first_b_feature = self.meta.n_confidences + self.meta.n_vectors * 2
        #     tensor_toconcat.append(x[:, :, first_b_feature:first_b_feature + self.meta.n_vectors])
        #     first_scale_feature = self.meta.n_confidences + self.meta.n_vectors * 3
        #     tensor_toconcat.append(x[:, :, first_scale_feature:first_scale_feature + self.meta.n_scales])
        #     import pdb; pdb.set_trace()
        #     x = torch.cat(tensor_toconcat, dim=2)
        ...
        if not self.training and self.inplace_ops:
            ...
            # TODO: CoreMLv4 problem (see above).
            x = torch.transpose(x, 1, 2)
        # print(f"conv output shape is {x.shape}")
        return x
