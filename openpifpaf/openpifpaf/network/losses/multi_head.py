...
class MultiHeadLoss(torch.nn.Module):
    ...
    def forward(self, head_fields, head_targets):  # pylint: disable=arguments-differ
        ...
        # print(f"self.losses is {self.losses[0]}")
        # print(f"head fields is {head_fields[0]}")
        # print(f"head targets[0] is {head_targets[0]}")
        # print(f"self.losses is {self.losses[0]}")
        # print(f"head fields is {torch.any(torch.isnan(head_fields[0]))}")
        # print(f"targets is {torch.any(torch.isnan(head_targets[0][0]))}")
        flat_head_losses = [ll
                            for l, f, t in zip(self.losses, head_fields, [head_targets[0],head_targets[1:]])
                            for ll in l(f, t)]
        # if self.training and head_fields[-1] is not None and head_fields[-2] is not None:
        #     depth_loss=mse_loss(head_fields[-2],head_fields[-1])
            # depth_loss=depth_loss.sum(d)
        #     # depth_loss=depth_loss.mean()
        # else:
        #     depth_loss=torch.tensor(0)
        # flat_head_losses.append(depth_loss)
        assert len(self.lambdas) == len(flat_head_losses),f"{len(self.lambdas)} and {len(flat_head_losses)}"
        loss_values = [lam * l
