...
from . import swin_transformer, xcit, vision_transformer
...
BASE_FACTORIES = {
    ...
    # ViT
    'vit_t': lambda: basenetworks.ViT('vit_t', vision_transformer.vit_tiny),
    'vit_s': lambda: basenetworks.ViT('vit_s', vision_transformer.vit_small),
    'vit_b': lambda: basenetworks.ViT('vit_b', vision_transformer.vit_base),
}
...
class Factory(Configurable):
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--mode', default='rgb', help='rgb or depth or comnine', choices=['rgb', 'depth', 'combine'])
        group.add_argument('--use-transformer', default=False, action='store_true', help='use transformer or not')
        group.add_argument('--depth-checkpoint', default=None, help='depth checkpoint path')
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        cls.mode = args.mode
        cls.use_transformer = args.use_transformer
        cls.depth_checkpoint=args.depth_checkpoint

    def factory(self, *, head_metas=None) -> Tuple[nets.Shell, int]:
        if self.base_name:
            assert head_metas
            assert self.checkpoint is None
            net_cpu: nets.Shell = self.from_scratch(head_metas, mode=self.mode, use_transformer=self.use_transformer,
    ...
    def consolidate_heads(self, net_cpu, head_metas):
        if self.head_consolidation == 'keep':
            ...
            if hasattr(net_cpu, 'depth_head_nets'):
                ...
                existing_headnets = {(hn.meta.dataset, hn.meta.name): hn
                                     for hn in net_cpu.depth_head_nets}
                headnets = []
                for meta_i, meta in enumerate(head_metas):
                    if (meta.dataset, meta.name) in existing_headnets:
                        hn = existing_headnets[(meta.dataset, meta.name)]
                        ...
                        headnets.append(
                            HEADS[meta.__class__](meta, net_cpu.base_net.out_features))
                net_cpu.set_depth_head_nets(headnets)
        else:
    ...
    def from_scratch(self, head_metas, mode, use_transformer,depth_checkpoint) -> nets.Shell:
        ...
        depth_net = BASE_FACTORIES[self.base_name]()
        net_cpu = nets.Shell(basenet, headnets, depth_net, mode=mode, use_transformer=use_transformer,depth_checkpoint=depth_checkpoint)
