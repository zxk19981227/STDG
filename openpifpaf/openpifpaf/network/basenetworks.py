...
class Resnet(BaseNetwork):
    ...
    def __init__(self, name, torchvision_resnet, out_features=2048):
        ...
        if self.inp_hm:
            self.inp_hm_conv1 = nn.Conv2d(self.inp_hm, 64, (3, 3), padding=1)
            self.inp_hm_conv2 = nn.Conv2d(64, 64, (3, 3), padding=1)
            self.inp_hm_relu = nn.ReLU()
    ...
    def forward(self, *args):
        ...
        if not inp_hm is None:
            d1 = self.inp_hm_conv1(inp_hm)
            d2 = self.inp_hm_conv2(self.inp_hm_relu(d1))
            x += d2
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--resnet-inp-hm',
                           default=None, type=int,
                           help='Size of input prior')
...
class SwinTransformer(BaseNetwork):
    ...
    def __init__(self, name, swin_net):
        ...
        self.backbone = swin_net(pretrained=self.pretrained,
                                 ...
                                 out_indices=out_indices, inp_hm=self.inp_hm)
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--swin-no-pretrain', dest='swin_pretrained',
                           default=True, action='store_false',
                           help='use randomly initialized models')
        group.add_argument('--swin-inp-hm',
                           default=None, type=int,
                           help='Size of input prior')
...
class ViT(BaseNetwork):
    ...
    def __init__(self, name, vit):
        ...
        super().__init__(name, stride=stride, out_features=self.out_features)
        self.backbone = vit(pretrained=self.pretrained, drop_path_rate=self.drop_path_rate)
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--vit-no-pretrain', dest='vit_pretrained',
                           default=True, action='store_false',
                           help='use randomly initialized models')
        group.add_argument('--vit-drop-path-rate', default=cls.drop_path_rate, type=float,
                           help="drop path (stochastic depth) rate")

    @classmethod
    def configure(cls, args: argparse.Namespace):
        cls.drop_path_rate = args.vit_drop_path_rate
        cls.pretrained = args.vit_pretrained
