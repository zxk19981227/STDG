...
def cli(parser):
    ...
    group_s.add_argument('--lr', type=float, default=1e-3,
                         help='learning rate')
    group_s.add_argument('--lr-basenet', type=float, default=None,
                         help='optional separate learning rate for the basenet')
    parser.add_argument('--lr-raf',default=None,type=float)
...
def factory_optimizer_named_params(args, named_parameters):
    # Added
    # if args.lr_basenet is not None and args.lr_raf is not None:
    #     param_dicts = [
    #         {"params": [p for n, p in named_parameters
    #                     if "base_net" not in n and p.requires_grad and "CompositeField" not in n]},
    #         {
    #             "params": [p for n, p in named_parameters
    #                        if "base_net" in n and p.requires_grad],
    #             "lr": args.lr_basenet,
    #             "init_lr": args.lr_basenet,
    #         },
    #         {
    #             "params": [p for n, p in named_parameters
    #                        if "CompositeField" in n and p.requires_grad],
    #             "lr": args.lr_raf,
    #             "init_lr": args.lr_raf,
    #         },
    #     ]
    if args.lr_basenet is not None:
        param_dicts = [
            {"params": [p for n, p in named_parameters
                        if "base_net" not in n and p.requires_grad]},
            {
                "params": [p for n, p in named_parameters
                           if "base_net" in n and p.requires_grad],
                "lr": args.lr_basenet,
