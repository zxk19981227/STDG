...
def _scale(image, anns, meta,depth, target_w, target_h, resample, *, fast=False):
    ...
    if fast and cv2 is not None:
        ...
        depth_np = cv2.resize(depth_np, (target_w, target_h), interpolation=cv_interpoltation)
        ...
        depth=depth.resize((target_w, target_h), resample)
    else:
        order = resample
        if order == 2:
            order = 1
        ...
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            depth_np = scipy.ndimage.zoom(depth_np, (target_h / h, target_w / w, 1), order=order)
        depth = PIL.Image.fromarray(depth_np)
...
class RescaleRelative(Preprocess):
    ...
    def __call__(self, image, anns, meta,depth):
        ...
        return _scale(image, anns, meta,depth, target_w, target_h, self.resample, fast=self.fast)
