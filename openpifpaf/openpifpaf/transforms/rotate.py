...
def rotate(image, anns, meta, angle,depth):
    ...
    if angle != 0.0:
        ...
        if depth_np.shape[0] == depth_np.shape[1] and angle == 90:
            depth_np = np.swapaxes(depth_np, 0, 1)
            depth_np = np.flip(depth_np, axis=0)
        elif depth_np.shape[0] == depth_np.shape[1] and angle == 270:
            depth_np = np.swapaxes(depth_np, 0, 1)
            depth_np = np.flip(depth_np, axis=1)
        elif depth_np.shape[0] == depth_np.shape[1] and angle == 180:
            depth_np = np.flip(depth_np, axis=0)
            depth_np = np.flip(depth_np, axis=1)
        else:
            fill_value = int(torch.randint(0, 255, (1,)).item())
            depth_np = scipy.ndimage.rotate(depth_np, angle=angle, cval=fill_value, reshape=False)
        depth = PIL.Image.fromarray(depth_np)
...
class RotateUniform(Preprocess):
    ...
    def __call__(self, image, anns, meta,depth):
        ...
        if abs(angle) > 1.0:
            ...
            image, anns, meta = center_pad(image, anns, meta,depth)

        return rotate(image, anns, meta, angle,depth)
