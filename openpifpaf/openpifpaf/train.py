...
def cli():
    ...
    parser.add_argument('--is-depth',default=False,help='[experiment] to whether to use the ')
    logger.cli(parser)
...
def main():
    args = cli()
    # c = ClearML_Singleton(args.output[8:])
    datamodule = datasets.factory(args.dataset)

    net_cpu, start_epoch = network.Factory().factory(head_metas=datamodule.head_metas)
    # pretrained_model= torch.load('/data1/zhouxukun/SGG-CoRF-main/openpifpaf/attention_weight/attention.epoch039.epoch060')['model'].state_dict()
    #
    # net_cpu.load_state_dict(pretrained_model,strict=False
    # )
    # freeze_net=[
    #     net_cpu.depth_encoder,net_cpu.base_net,net_cpu.depth_to_rgb,net_cpu.head_nets
    # ]
    # for net in freeze_net:
    #     print(f"frezzing net is {net}")
    #     for param in net.named_parameters():
    #         param[1].requires_grad = False
    # for param in net_cpu.named_parameters():
    #     param[1].requires_grad=True
    if args.mode=='depth':
        loss = network.losses.Factory().factory(net_cpu.depth_head_nets)
    elif args.mode=='rgb':
    ...
    if args.lr_basenet is not None:
        optimizer = optimize.factory_optimizer_named_params(
            args, list(net.named_parameters()) + list(loss.named_parameters()))
    else:
